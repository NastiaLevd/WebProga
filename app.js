...
function generateRandomTrainer() {
    const names = ["Алексей Громов", "Марина Власова", "Игорь Степнов"]
    const specs = ["кроссфит", "бассейн", "единоборства", "реабилитация"]
    return {
        name: names[Math.floor(Math.random() * names.length)],
        experience: Math.floor(Math.random() * 15) + 1,
        isAvailable: Math.random() > 0.3,
        hireDate: new Date().toISOString().split('T')[0],
        specializations: specs.slice(0, Math.floor(Math.random() * 2) + 1)
    }
}

function generateRandomMember() {
    const names = ["Дмитрий Н.", "Ольга К.", "Артем П."]
    const goals = ["похудение", "набор массы", "выносливость"]
    return {
        fullName: names[Math.floor(Math.random() * names.length)],
        age: Math.floor(Math.random() * 40) + 18,
        hasMembership: Math.random() > 0.5,
        membershipStart: new Date().toISOString().split('T')[0],
        trainings: goals.slice(0, Math.floor(Math.random() * 2) + 1)
    }
}
...
app.get('/trainers', async (ctx) => {
    const trainers = await readJSON(TRAINERS_FILE)
    ctx.send(trainers)
})

app.get('/trainers/:id', async (ctx) => {
    const trainers = await readJSON(TRAINERS_FILE)
    const trainer = trainers.find(t => t.id == ctx.req.params.id)
    trainer ? ctx.send(trainer) : ctx.sendError('Тренер не найден', 404)
})

app.post('/trainers', async (ctx) => {
    const trainers = await readJSON(TRAINERS_FILE)
    const newTrainer = Object.keys(ctx.req.body).length === 0 ? generateRandomTrainer() : ctx.req.body
    newTrainer.id = trainers.length > 0 ? Math.max(...trainers.map(t => t.id)) + 1 : 1
    trainers.push(newTrainer)
    await writeJSON(TRAINERS_FILE, trainers)
    ctx.send(newTrainer, 201)
})

app.delete('/trainers/:id', async (ctx) => {
    let trainers = await readJSON(TRAINERS_FILE)
    trainers = trainers.filter(t => t.id != ctx.req.params.id)
    await writeJSON(TRAINERS_FILE, trainers)
    ctx.send({ message: 'Тренер удален' })
})

app.get('/members', async (ctx) => {
    const members = await readJSON(MEMBERS_FILE)
    ctx.send(members)
})

app.post('/members', async (ctx) => {
    const members = await readJSON(MEMBERS_FILE)
    const newMember = Object.keys(ctx.req.body).length === 0 ? generateRandomMember() : ctx.req.body
    newMember.id = members.length > 0 ? Math.max(...members.map(m => m.id)) + 1 : 1
    members.push(newMember)
    await writeJSON(MEMBERS_FILE, members)
    ctx.send(newMember, 201)
})
